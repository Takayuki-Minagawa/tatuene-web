...
## 入力支援機能
- **表示倍率**: 縮小/拡大、「全体」（シート全体をペインに収める）、「100%」。シート切替後も維持。
- **入力モード**: 入力欄を含む行だけに絞って表示（評価シートは対象外）。
- **見出し固定**: 列見出しのあるシート（部材性能シート）はスクロールしても見出しが残る。
- **未入力ジャンプ**: 空欄数を表示し「次へ」で順に移動。空欄はセル左に色帯で強調。
- **キーボード操作**: 入力欄で Enter/Shift+Enter（前後の欄）・上下矢印（同列の隣行）で移動。
- **ドラフト自動保存**: 編集内容は端末内（localStorage＋IndexedDB）に自動保存され、次回起動時に復元を確認。
  ※サーバー送信はなし。共有端末では「↺ 初期化」で消去できる（個人情報の残留に注意）。
- **元に戻す/やり直し**: 図面の変形・注釈に対応。
...
- **PDF帳票はラスター画像**（html2canvasで評価シートを画像化してA4に貼付）。文字の選択・検索は不可。巨大化時は倍率を自動で下げる。
- 入力欄に `=` で始まる値を入れると数式として扱われる（計算結果が下流に反映され、原文は保存ファイルに保持される）。

> 補足: 開発の初期計画は `docs/作業計画.md`（着手時点のもので現状と一部相違あり）。本READMEが現状の正。
